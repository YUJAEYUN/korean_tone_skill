...
- 진단 카탈로그(활용): [`references/ai-tell-catalog.md`](./references/ai-tell-catalog.md)
...
- 예문 뱅크: [`examples/pairs/`](./examples/pairs/) (Before/After, 진행 중 — 100~200쌍 목표)
- 변경률 가드 스크립트: [`scripts/change_rate_check.py`](./scripts/change_rate_check.py)

## 설계 원칙

1. 바퀴를 다시 만들지 않는다 — AI체 진단은 검증된 외부 카탈로그를 그대로 활용한다.
2. 직접 만들 가치가 있는 부분("그럼 어떻게 써야 하는가")에만 집중한다.
3. 구조와 표현을 분리한다 — 수능 비문학은 논리 구조 학습용으로만 쓰고, 문장 표현의
   모범으로는 쓰지 않는다.
4. 의미 보존이 최우선이다 — 사실관계·수치·주장 방향은 절대 바꾸지 않는다.
...
- [x] `examples/pairs/` 시드 15쌍 (장르별 5쌍)
- [x] 변경률 가드 스크립트
- [ ] 예문 뱅크를 100~200쌍으로 확장 (최우선)
